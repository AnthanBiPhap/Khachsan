import { env } from "../constanst/getEnvs";

export const fetchRoomTypes = async (page = 1, limit = 10) => {
  try {
    const res = await fetch(`${env.API_URL}/api/v1/roomTypes?page=${page}&limit=${limit}`);
    if (!res.ok) {
      const error = await res.json().catch(() => ({}));
      throw new Error(error.message || "Failed to fetch room types");
    }
    const response = await res.json();
    const { data } = response;

    return {
      data: data?.roomTypes || [],
      pagination: {
        page: data?.pagination?.page ?? page,
        limit: data?.pagination?.limit ?? limit,
        total: data?.pagination?.totalRecord ?? 0,
      },
    };
  } catch (error) {
    console.error("Error fetching room types:", error);
    return {
      data: [],
      pagination: { page, limit, total: 0 },
    };
  }
};

export const deleteRoomType = async (id: string) => {
  const res = await fetch(`${env.API_URL}/api/v1/roomTypes/${id}`, { method: "DELETE" });
  if (!res.ok) throw new Error("Failed to delete room type");
  return true; // assume 204 No Content
};
