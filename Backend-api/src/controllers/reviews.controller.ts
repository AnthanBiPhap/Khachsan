...
const getAll = async(req: Request, res: Response, next: NextFunction) => {
    try{
        const users = await reviewService.getAll(req.query);
...
const getById = async(req: Request, res: Response, next: NextFunction) => {
    try{
        ...
        const { id } = req.params;
        const user = await reviewService.getById(id);
...
const Create = async(req: Request, res: Response, next: NextFunction) => {
    try{
        ...
        const user = await reviewService.create(payload);
...
const Update = async(req: Request, res: Response, next: NextFunction) => {
    try{
        const { id } = req.params;
        const payload = req.body;
        const user = await reviewService.updateById(id, payload);
        sendJsonSuccess(res, user, httpStatus.OK.statusCode, httpStatus.OK.message);
    }
    catch(error) {
        next (error);
...
const Delete = async(req: Request, res: Response, next: NextFunction) => {
    try{
        const { id } = req.params;
        const user = reviewService.deleteById(id);
        res.status(204).json({
            user,
